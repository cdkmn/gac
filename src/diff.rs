...
fn score_file(path: &str) -> u8 {
    ...
    // Check path components for test directories, and file name for test naming
    // conventions — avoids false positives on unrelated files like contest.rs.
    let is_test_path = path_buf.iter().any(|component| {
        let c = component.to_string_lossy();
        c == "test" || c == "tests" || c == "__tests__"
    }) || path_buf.file_name().is_some_and(|f| {
        let name = f.to_string_lossy();
        name.contains("_test") || name.contains(".spec")
    });
    if is_test_path {
        return 40;
    }
